...
//import generic.interfaceEx.question.Main;
import java.util.ArrayList;
import java.util.List;
import lambda.day02.innerclass2.A;
...
public class GsonUtil {
    ...
    private  static Gson gson2 = new GsonBuilder()
            .disableHtmlEscaping()   //HTML 문자를 유니코드로 이스케이프 처리한다. GsonBuilder 를 통해 생성된 객체가 HTML문자를 그대로 전달하겠다.
            .setPrettyPrinting()    // Json 문자열을 가독성 좋게 출력하게 하는 옵션
            .serializeNulls()       //기본적으로 Gson 은 null 필드를 생략하지만, 해당 옵션을 사용하면 null로 인해 필드가 누락된 경우 발생할 수 있는 예외상황을 예방할 수 있다.
            .create();

    private static String getJsonData(){
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("data","Test Data <신세계 자바 백엔드 개발자 과정>");
        //jsonObjet를 Json 문자열로 변환 (toJson())
